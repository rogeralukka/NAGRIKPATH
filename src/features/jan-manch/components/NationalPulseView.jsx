 ...
 * - 4 High-impact macro vital tiles styled with strict binary chromatic theme
 * - "Desh Ka Index" Composite Sovereign Progress Hero visualizer (computed at runtime)
 * - 12-Card Responsive Recharts Grid with independent local scrubbers
 * - 100% Offline Client-Side Verification Fallback
 */
export default function NationalPulseView() {
  ...
  const currentInrInverse = (1 / currentInrRate).toFixed(4);

  const gstTheme = getMetricColorTheme(124.1, false);
  const upiTheme = getMetricColorTheme(787.0, false);
  const forexTheme = getMetricColorTheme(22.3, false);
  const inrTheme = getMetricColorTheme(-22.7, true);
  ...
  const macroVitals = [
    {
      ...
      unit: "Cr / mo",
      delta: "↑ 124.1% vs FY21",
      citation: "Source: GST Council, Ministry of Finance",
      icon: TrendingUp,
      isPositive: true,
      theme: gstTheme
    },
    {
      ...
      unit: "Billion Txns",
      delta: "↑ 787.0% vs FY21",
      citation: "Source: National Payments Corporation of India",
      icon: Zap,
      isPositive: true,
      theme: upiTheme
    },
    ...
    },
    {
      ...
      unit: "/ USD",
      subtext: `Rupee Purchasing Power (1 INR = $${currentInrInverse} USD)`,
      delta: "↓ 22.7% vs FY21",
      citation: "Source: Market benchmark",
      icon: DollarSign,
      isPositive: false,
      theme: inrTheme
    }
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-150">
          ...
          return (
            ...
            >
              ...
              <div className="my-2">
                ...
                <div
                  className={`inline-flex items-center gap-1 whitespace-nowrap shrink-0 px-2 py-0.5 rounded-md text-[11px] font-semibold leading-none border mt-1 ${theme.badgeClass}`}
                >
                  {vital.isPositive ? (
      ...
      {/* 2. Deliverable 2: "Desh Ka Index" Composite Sovereign Progress Hero */}
      ...
      <div className="pt-2 border-t border-slate-200 dark:border-white/[0.08]">
        <h2 className="text-sm font-bold uppercase tracking-wider text-slate-900 dark:text-[#EDEDED] font-mono">
          MACROECONOMIC &amp; DIGITAL MATRIX
        </h2>
        <p className="text-xs text-slate-500 dark:text-[#8A8F98] mt-0.5">
          12 empirical indicators tracking national progress
        </p>
      </div>

      {/* 4. 12-Card Responsive Recharts Grid */}
